...
def analyze_sbox(req: SBoxRequest):
    ...
    try:
        ...
        return {
            "nl": nl, "sac": sac, "bic_nl": bic_nl, "bic_sac": bic_sac,
            "du": du, "dap": dap, "lap": lap, "ad": ad, "ci": ci, "to": to, "sv": sv
        }
    except Exception as e:
...
def encrypt_text(req: EncryptTextRequest):
    ...
    if len(req.key) != 16:
        raise HTTPException(status_code=400, detail="Key harus tepat 16 karakter (128-bit)")
    
    try:
        key_bytes = req.key.encode('utf-8')
        inv_sbox = sbox_logic.generate_inverse_sbox(req.sbox)
        cipher_hex = aes_cipher.encrypt_text(key_bytes, req.plaintext, req.sbox, inv_sbox)
...
def decrypt_text(req: DecryptTextRequest):
    ...
    try:
        key_bytes = req.key.encode('utf-8')
        inv_sbox = sbox_logic.generate_inverse_sbox(req.sbox)
        plaintext = aes_cipher.decrypt_text(key_bytes, req.ciphertext_hex, req.sbox, inv_sbox)
        return {"plaintext": plaintext}
    except Exception as e:
        raise HTTPException(status_code=500, detail="Gagal dekripsi. Cek Key atau Ciphertext.")
...
def process_image(req: EncryptImageRequest, mode: str = "encrypt"):
    ...
    try:
        ...
        if mode == "encrypt":
            ...
            # Enkripsi P2 -> C2
            c2 = aes_cipher.encrypt_data(key_bytes, bytes(input_array), req.sbox, inv_sbox)
            ...
            result_bytes = aes_cipher.decrypt_data(key_bytes, input_bytes, req.sbox, inv_sbox)
